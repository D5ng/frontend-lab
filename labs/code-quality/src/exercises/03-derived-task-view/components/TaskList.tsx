  ...
  return visibleTasks.length > 0 ? (
    <FlexBox as="ul" className="daily-task-list" flexDirection="column">
      {visibleTasks.map((task) => (
        <FlexBox as="li" alignItems="center" className="daily-task-item" data-completed={task.completed} gap="12px" key={task.id}>
          <CheckMark
          ...
          <FlexBox className="daily-task-item-body" flexDirection="column" gap="5px">
            <Typography as="h3" className="daily-task-title" variant="body1" weight="medium">
              {task.title}
            </Typography>
            <FlexBox className="daily-task-metadata" flexWrap="wrap" gap="4px 8px">
              <Typography as="span" className="daily-task-category" color="semantic.label.alternative" variant="caption1">
                {CATEGORY_LABELS[task.category]}
              </Typography>
              <Typography as="span" className="daily-task-meta-detail" color="semantic.label.alternative" variant="caption1">
                {task.due}
              </Typography>
              <Typography as="span" className="daily-task-meta-detail" color="semantic.label.alternative" variant="caption1">
                {task.place}
